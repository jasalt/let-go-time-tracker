...
func invokeArgs(context string, fn vm.Fn, args ...vm.Value) bool {
	return gtkbridge.Invoke(context, fn, args...)
}

func invoke(context string, fn vm.Fn) bool { return invokeArgs(context, fn) }
...
func installHost(lg *api.LetGo) error {
	defs := map[string]any{
		...
		"gtk-hbox": func(spacing int) *gtk.Box {
			return gtk.NewBox(gtk.OrientationHorizontal, spacing)
		},
		"gtk-entry":           gtk.NewEntry,
		"gtk-check-button":    gtk.NewCheckButton,
		"gtk-dropdown":        func() *gtk.DropDown { return gtk.NewDropDown(nil, nil) },
		"gtk-scrolled-window": gtk.NewScrolledWindow,
		...
		"set-child-any!": func(parent, child vm.Value) {
			widget := child.Unbox().(gtk.Widgetter)
			switch p := parent.Unbox().(type) {
			case *gtk.ApplicationWindow:
				p.SetChild(widget)
			case *gtk.ScrolledWindow:
				p.SetChild(widget)
			default:
				panic(fmt.Sprintf("unsupported single-child parent %T", p))
			}
		},
		"clear-child-any!": func(parent, _ vm.Value) {
			switch p := parent.Unbox().(type) {
			case *gtk.ApplicationWindow:
				p.SetChild(nil)
			case *gtk.ScrolledWindow:
		...
		"button-label!": func(button vm.Value, text string) {
			button.Unbox().(interface{ SetLabel(string) }).SetLabel(text)
		},
		"entry-text!":        func(entry *gtk.Entry, text string) { entry.SetText(text) },
		"entry-placeholder!": func(entry *gtk.Entry, text string) { entry.SetPlaceholderText(text) },
		"check-active!":      func(button *gtk.CheckButton, active bool) { button.SetActive(active) },
		"dropdown-items!": func(dropdown *gtk.DropDown, items []string) {
			dropdown.SetModel(gtk.NewStringList(items))
		},
		"dropdown-selected!": func(dropdown *gtk.DropDown, index int) { dropdown.SetSelected(uint(index)) },
		...
		"on-click!": func(button *gtk.Button, fn vm.Fn) glib.SignalHandle {
			return button.ConnectClicked(func() { invoke("clicked", fn) })
		},
		"on-entry-change!": func(entry *gtk.Entry, fn vm.Fn) glib.SignalHandle {
			return entry.ConnectChanged(func() { invokeArgs("changed", fn, vm.String(entry.Text())) })
		},
		"on-toggle!": func(button *gtk.CheckButton, fn vm.Fn) glib.SignalHandle {
			return button.ConnectToggled(func() { invokeArgs("toggled", fn, vm.Boolean(button.Active())) })
		},
		"on-dropdown-change!": func(dropdown *gtk.DropDown, fn vm.Fn) glib.SignalHandle {
			return dropdown.NotifyProperty("selected", func() {
				invokeArgs("selected", fn, vm.Int(dropdown.Selected()))
			})
		},
		...
		"app-run!": func(app *gtk.Application) int { return app.Run([]string{"gtt-letgo-gtk"}) },
	}
...
func newRuntime() (*api.LetGo, error) {
	lg, err := api.NewLetGo("gtt-letgo-gtk")
	if err != nil {
		return nil, err
	}
...
func run() error {
	...
	for index, source := range embeddedSources {
		if err := lg.Def("__embedded_source", source); err != nil {
			return fmt.Errorf("define embedded source %d: %w", index, err)
		}
		if _, err := lg.Run("(load-string __embedded_source)"); err != nil {
			return fmt.Errorf("load embedded source %d: %w", index, err)
		}
	}
	if _, err := lg.Run("(in-ns 'gtt-letgo-gtk)"); err != nil {
...
func main() {
	...
	if err := run(); err != nil {
		fmt.Fprintln(os.Stderr, "gtt-letgo-gtk:", err)
		os.Exit(1)
	}
}
